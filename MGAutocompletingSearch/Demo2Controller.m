...
@interface Demo2Controller ()<MGAutocompletingSearchViewDelegate>
...
- (void)setupViewUI {
    ...
    [self.searchView.suggestTable registerNib:[UINib nibWithNibName:NSStringFromClass([UserCell class]) bundle:nil] forCellReuseIdentifier:NSStringFromClass([UserCell class])];
...
- (UITableViewCell *)searchView:(MGAutocompletingSearchView *)searchView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UserCell *cell = [searchView.suggestTable dequeueReusableCellWithIdentifier:NSStringFromClass([UserCell class]) forIndexPath:indexPath];
    cell.nameLabel.text = [self.suggestItems objectAtIndex:indexPath.row];
